...
export class StudentCommentsService{
  ...
  public static async update(obj: FormData, id: number):Promise<any>{
    ...
    if(!localStorage.getItem('token')){
      ...
      resp = await SimeService.post('/students/comments/update/' + id, obj, { Authorization: 'Bearer ' + localStorage.getItem('token') });
      if (resp.status === 200){
          return resp.data;
      } else{
          return resp.data;
  ...
  public static async get(id: number):Promise<any>{
    ...
    if(!localStorage.getItem('token')){
      ...
      resp = await SimeService.get('/students/comments/' + id, { Authorization: 'Bearer ' + localStorage.getItem('token') });
      if (resp.status === 200){
          return resp.data;
      } else {
          return resp.data;
